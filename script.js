...
const MobileNavItems = gsap.utils.toArray(".mobile-nav-items .item");

MobileNavItems.forEach((NavItem) => {
  const NavData = NavItem.querySelector(".item-data");
  const DropDown = NavItem.querySelector(".drop-down");
  NavItem.addEventListener("click", (e) => {
    let IsClosed = NavItem.getAttribute("closed") == "true";
    const DataBounds = NavData.getBoundingClientRect();
    if (IsClosed) {
      gsap.to(NavItem, {
        height: DataBounds.height + 60,
      });
      gsap.to(DropDown, {
        rotate: 180,
      });
      NavItem.setAttribute("closed", "false");
    } else if (!IsClosed) {
      gsap.to(NavItem, {
        height: 50,
      });
      ...
      NavItem.setAttribute("closed", "true");
    }
  });
});
